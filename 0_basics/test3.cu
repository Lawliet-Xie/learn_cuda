...
__global__ void gpu_matrix_mul(int *a, int *b, int *c, const int size)
{
...
void cpu_matrix_mul(int *a, int *b, int *c, const int size)
{
...
int main()
{
    ...
    dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE); // 1. gpu warp 32 2. thread num: 32倍数 <=1024

    gpu_matrix_mul<<<dimGrid, dimBlock>>>(d_a, d_b, d_c, matrix_size);

    cudaMemcpy(h_c, d_c, memsize, cudaMemcpyDeviceToHost);

    cpu_matrix_mul(h_a, h_b, h_cc, matrix_size);
